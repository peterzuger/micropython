...
async def https_get(url, port, sslctx):
...
async def http_https_get(url1, url2, port, sslctx):
    await https_get(url2, port, client_ctx)
    await http_get(url1)


asyncio.run(http_https_get("micropython.org", "micropython.org", 443, client_ctx))
